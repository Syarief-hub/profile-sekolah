    ...
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <h2 class="font-bold text-2xl text-slate-800 leading-tight">
                <i class="fa-solid fa-envelope text-blue-600/70"></i> {{ __('Pesan Masuk') }}
            </h2>
        </div>
    </x-slot>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-blue-50/50 transition duration-150">
                                ...
                                <td class="py-4 px-6 font-medium text-slate-800 flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-xs uppercase">{{ substr($contact->name, 0, 1) }}</div>
                                    {{ $contact->name }}
                                </td>
                                <td class="py-4 px-6 font-medium text-slate-700 truncate max-w-xs">{{ $contact->subject }}</td>
                                <td class="py-4 px-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.contacts.show', $contact->id) }}" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white flex items-center justify-center transition tooltip" title="Baca Pesan">
