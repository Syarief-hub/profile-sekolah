    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-bold text-2xl text-slate-800 leading-tight">
                {{ __('Tambah Data Ekstrakurikuler') }}
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto">
            <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden">
                <div class="p-6 sm:p-10">
                    ...
                    <form method="POST" action="{{ route('admin.extracurriculars.store') }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="name" name="name" 
                                    ...
                                    placeholder="Contoh: Bola Voli / Pramuka / PMR" value="{{ old('name') }}" required>
                            ...
                            <div>
                                ...
                                <select id="category" name="category" required class="w-full rounded-xl border-slate-200 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 transition-colors py-3">
                                    <option value="Wajib" {{ old('category') == 'Wajib' ? 'selected' : '' }}>Wajib</option>
                                    ...
                                    <option value="Akademik / Sains" {{ old('category') == 'Akademik / Sains' ? 'selected' : '' }}>Akademik / Sains</option>
                                    <option value="Kedisiplinan & Bela Negara" {{ old('category') == 'Kedisiplinan & Bela Negara' ? 'selected' : '' }}>Kedisiplinan & Bela Negara</option>
                            ...
                            <!-- Jadwal -->
                            <div>
                                <label for="schedule" class="block text-sm font-bold text-slate-700 mb-1">Jadwal Pelaksanaan (Opsional)</label>
                                <input type="text" id="schedule" name="schedule" 
                                    class="w-full rounded-xl border-slate-200 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 transition-colors py-3" 
                                    placeholder="Contoh: Setiap Jumat, 15:30 WITA" value="{{ old('schedule') }}">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3"
                                    ...
                                    placeholder="Jelaskan secara singkat kegiatan dari ekstrakurikuler ini...">{{ old('description') }}</textarea>
                                @error('description')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                            
                            <!-- Foto -->
                            <div class="md:col-span-2">
                                <label for="image" class="block text-sm font-bold text-slate-700 mb-1">Foto Sampul / Kegiatan (Opsional)</label>
